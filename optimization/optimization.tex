...
Farmer Bob has 2400 linear feet of fence. He wishes to build a rectangular enclosure with a partition fence 
...
To maximize the area, Farmer Bob should use all 2400 linear feet of fence.  Since there are two horizontal 
segments of length $x$ and three vertical segments of length $y$ (because of the partition), we 
get the {\it constraint} equation:
\[2x+3y=2400.\]
...
Since $x$ represents a length of fence, we have $x \geq 0$. Furthermore, since we have 2400 feet of 
fence $2x \leq 2400$ which implies $x\leq 1200$.
Thus, we would like to find the absolute maximum of $A(x) = 800x - \frac23 x^2$ on the interval $[0, 1200]$.
We use the closed interval method of section 3.2 to find the absolute maximum.
...
Thus, the absolute maximum area occurs when $x = 600$ ft. 
...
Plugging $x = 600$ into the constraint equations gives us the corresponding $y$-value:
...
Let $x$ be the length of the enclosure and $y$ the width. Because of the partitions, there 
are three horizontal and three vertical sections of fence. Hence, the total amount of fence needed can be expressed as 
\[F = 3x + 3y.\]
...
A cardboard box with a square base and an open top is to be constructed so that it has a volume of 4000 cubic inches. 
Find the minimum amount of cardboard needed to achieve this construction.
\begin{hint}
Let $x$ be the length and width of the square base
\end{hint}
\begin{hint}
...
\end{hint}
\begin{hint}
The volume constraint is $x^2 y = 4000$
\end{hint}
\begin{hint}
The amount of cardboard needed is $A = x^2 + 4xy$; replace $y$ using the volume constraint
\end{hint}
\begin{hint}
Set the derivative of $A$ equal to zero
\end{hint}

The optimal length and width are $x = \answer{20}$ inches.\\
The optimal height is $y = \answer{10}$ inches.\\
The minimum amount of cardboard needed is $ \answer{1200}$ square inches.
\end{problem}

In some problems, the objective function involves a composition, $f(g(x))$.  
If the outside function $f$ is strictly increasing, then the max or min of the composition 
occurs at the same $x$-value as the max/min of $g(x)$. 
...
We will assume that the sun is located at the origin and that the path of the comet 
follows the parabola $y = x^2 - 5$. The units 
...
A cheetah is on the bank of a 50-meter wide, straight river.   The cheetah spots prey on the opposite 
bank of the river, 100 meters upstream. 
The cheetah decides to walk along the rivers edge for part of the way before swimming directly at the prey. 
If the cheetah walks at 2 m/s and swims at 1 m/s,
how far should the cheetah walk before swimming so as to minimize the total time to get to the prey?\\

Let $100 -x$ meters be the distance that the cheetah walks and $y$ meters the distance that the cheetah swims, 
...
the total time is 
\[
T = \frac{100-x}{2} + \frac{y}{1}.
\]
This is the objective equation.
The constraint comes from the Pythagorean Theorem. The swim portion of the trip is along the hypotenuse of a right triangle 
with legs parallel and perpendicular to the river bank. One leg of the triangle is $50$ and the other is $x$.  
...
Substituting this into the objective gives:
\[
T(x) = \frac{100-x}{2} + \sqrt{50^2 + x^2},\]
where $0 \leq x \leq 100$.
To find the minimum value of $T$, we find the critical numbers (using the chain rule):
\[
T'  = -\frac12 + \frac{x}{\sqrt{50^2 + x^2}} = 0
\]

Solving for $x$ gives
\[
\frac{x}{\sqrt{50^2 + x^2}} = \frac12
\]
\[
2x = \sqrt{50^2 + x^2}.
\]
...
\]
\[
x = \frac{50}{\sqrt{3}} \approx 28.87 \text{ meters}.
\]


To verify that this is $x$-value gives the minimum time, we can use the closed interval method from 
section 3.2 (Extreme Value Theorem).
We compare the values of $T = \frac{100-x}{2} + \sqrt{50^2 + x^2}$ at the critical number and the endpoints of the interval:
\[
T(0) = 50 +  50 = 100 
\]
\[
T(100) =  \sqrt{50^2 + 100^2} = 50\sqrt 5 \approx 111.80
\]
\[
T(50/\sqrt 3) = 50 - \frac{25}{\sqrt{3}} +  \sqrt{50^2 + \frac{50^2}{3}} = 50 + 25\sqrt{3} \approx 93.3
\]
The minimum time to reach the prey is  $T \approx 93.3$ seconds, achieved when the cheetah 
walks $100 -x \approx 71.13$ meters, and
then swims directly towards the prey.
\end{example}
...
What is the minimum amount of time it will take her to reach her destination if she runs at 8 km/hr and rows at 6 km/hr?
