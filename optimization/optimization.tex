...
\begin{center}
\begin{tikzpicture}
\node at (5, 6.5) {Area = length $\times$ width};
\draw (0,0) --(10, 0) -- (10, 6) -- (0, 6) -- (0, 0);
\draw[<->] (10.5, 0) --(10.5,6) node[right, midway]{y};
\draw (6, 0) -- (6, 6);
\draw[<->] (0, -.5) --(10,-.5) node[below, midway]{x};
\end{tikzpicture}
\end{center}
...
\begin{image}
\begin{tikzpicture}
\begin{axis}[axis lines = center,  axis y line=center,  xmin=-0.5, xmax = 4.5, xtick={2}, , ytick={0}, yticklabels={,,}, xticklabel={$600$ ft.}, xlabel=$x$, ylabel=$A$, ymin = -0.5, ymax = 4.5, 
