...
The Cauchy-Riemann Equations tell us that if $f$ is a differentianiable function of $z$, then it must satisfy $f_x = -if_y$.
There is also a partial converse to this statement.  

\begin{theorem}
If the partial derivatives $u_x, u_y, v_x$ and $v_y$ are continuous at a point $(x_0,y_0)$, 
and if these partial derivatives satisfy the Cauchy-Riemann Equations at this point, then $f(z) = u(x,y) + iv(x,y)$
...
\]
In a standard partial derivative, on of $h$ or $k$ would be zero. Our goal is to turn this limit into partial derivatives.
We will do so using the Mean Value Theorem
\[
g(b) -g(a) - g'(c)(b-a)
\]
where $g$ is a differentiable function (of a single variable) and $c$ is between $a$ and $b$. In the limit above, 
we will add and subtract the term $u(x,y+k)$.
This leads to 
\[
...
considering $u$ to be a function of a single variable (since the second coponent is constant).
\[
...
\[
L_1 = \lim_{h+ik \to 0} \frac{u_x(x,y)h}{h+ik} + \lim_{h+ik \to 0} \frac{[u_x(x*,y+k)-u_x(x,y)]h}{h+ik}= L_1' + L_1''
\]
In the limit $L_1``$, note that $\left|\frac{h}{h+ik}\right| \leq 1$, hence
...
\lim_{h+ik \to 0} \left|u_x(x^*,y+k)-u_x(x,y)\right| =0
\]
Finally, the Squeeze Theorem gives $L_1'' = 0$ and so
\[
...
\[
L_4 = -i^2\lim_{h+ik \to 0} \frac{u_x(x,y)k}{h+ik} = -i\lim_{h+ik \to 0} \frac{u_x(x,y)ik}{h+ik}
\]
and 
\[
