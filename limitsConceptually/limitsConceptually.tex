...
the indeterminate form $0/0$.  To find this limit analytically, we will factor the numerator 
...
This example used the \textbf{factor and cancel} method.
\end{example}
...
\begin{question} %problem #2a
  Compute the following limit which we investigated numerically in the previos section:
  \[
...
\[\text{If} \quad p(a) = 0 \quad \text{then} \quad  x-a \text{is a factor of} \quad p(x).\]
...
\[ 4 + \sqrt{13 -x}.\]
To simplify our calculations a little bit, lets multiply the conjugates together separately:
...
\\
Plugging in $x=4$ yields the familiar $0/0$ indeterminate form. The algebra skills necessary to transform the 
...
\[ \frac{a-b}{b-a} = -1. \]
The the ratio of opposites is $-1$.
\end{example}
...
Plugging in $x = 3$ gives the indeterminate form $0/0$. To resolve this limit, 
