...
%\[\int f(g(x))g'(x) \ dx = \int f(u) \ du \] which equals
%\[F(u) + C\] and back substituting gives \[F(g(x)) + C. \]  Now we can see that \[\int f(g(x))g'(x) \ dx = F(g(x)) + C. \]
...
dx = \frac{1}{2x} \; du.
\]
We can now proceed with step one and convert the original intergal into a simpler one, in the variable, $u$:
\[
