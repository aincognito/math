...
Moreover, we know exactly where, since $f'(x) = \cos(x)$ and the equation $\cos(x) = 0$ has a solution at $x = \pi/2$, which is in the interval.
\end{example}
...
\]
We would like to demonstrate that $p(x)$ can have at most 3 roots.
%So we would like to show that an equation of the form $p(x) = 0$, where $p(x)$ is a cubic polynomial, 
Now argue by \it{contradiction}. 
%We  suppose that $(p(x)$ can have 4 roots and draw from that an erroneous cocnlusion.
...
Suppose that a cubic polynomial, $p(x)$, has 4 roots.  From smallest to largest, we label the 4 roots $x_1, x_2, x_3$ and $x_4$.
...
that $p'(x)$ has a root on each of these intervals. That is, there exist numbers $c_1, c_2$ and $c_3$ such that
\[
p'(c_1) = p'(c_2) = p'(c_3) = 0,
\]
and
\[
x_1 < c_1 < x_2, \quad x_2 < c_2 < x_3 \quad \text{and} \quad x_3 < c_3 < x_4.
\]
\begin{image}
\begin{tikzpicture}
...
But $p'(x)$ cannot have 3 roots. Since $p(x)$ is a cubic polynomial, the power rule tells us that $p'(x)$ is a quadratic polynomial. 
As such, $p'(x)$ can have at most 2 roots.
This contradiction implies that a cubic polynomial cannot have 4 roots. Thus, a cubic polynomial can have at most 3 roots.\\
In fact, more generally a polynomial of degree $n$ can have at most $n$ roots.  
This can be proven using an arguemnt similar to the one used in this example and an advanced technique known as principle of {\it mathematical induction}.
\end{example}
