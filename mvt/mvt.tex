...
\addplot[domain=4.3:5.7, 
    ...
    \node at (axis cs:3, -1.75) {The roots of $p'(x)$ are $c_1, c_2$ and $c_3$};
...
As an example of the conceptual interpretation of the theorem, consider a car that 
averages a speed of 47.4 miles per hour on a long trip. 
...
The Mean Value Theorem is an {\it existence} theorem because it asserts that there exists at least one 
value $x=c$ inside the interval $(a,b)$
...
%Then find all values of $x$ that satisfy the conclusion of the theorem.\\
Since $x^2$ is a polynomial, it is continuous and differentiable everywhere. Therefore, $f$ is continuous on the 
closed interval $[0, 3]$ and differentiable on the open interval $(0, 3)$. 
...
When applying the MVT to a quadratic polynomial on any interval $[a, b]$ the value of $x$ that satisfies the 
theorem will always be the mid-point!

In the figure below, the secant line is in blue, 
and the tangent line at $x = 3/2$ is in red. The Mean Value Theorem asserts that these lines are parallel, 
and the figure makes this clear.
...
Here is a video solution of  problem 4c:
\begin{foldable}
\youtube{fFR7AF-PpE0}
\end{foldable}
