...
\end{tikzpicture}
\end{image}
If $f$ is a constant function on $[a,b]$, then $f'(x)= 0$ for all $x$ in the open interval $(a,b)$ and the theorem is true.
Now, suppose that $f$ is not constant on the closed interval $[a,b]$.
By hypothesis, $f$ is continuous on the closed interval $[a,b]$, and therefore we can apply to the Extreme Value Theorem (section 3.2) to conclude
that $f$ has absolute extremes on $[a,b]$.
Since the theorem requires that $f(a) = f(b)$, we can conclude that the non-constant function, $f$, must have an absolute extreme in the open interval $(a,b)$.
By Fermat's Theorem (section 3.2), this absolute extreme occurs at a critical number, $x= c$. Finally, since $f$ is differentiable on $(a,b)$ (by hypothesis), we can conclude 
that $f'(c) = 0$. Thus, the theorem is true in this case as well, and Rolle's Theorem is proved.
...
Suppose that there is cubic polynomial, $p(x)$, that has four roots.  Label these roots in ascending order: $x_1 <  x_2 < x_3 < x_4$.
...
p'(c_1) = p'(c_2) = p'(c_3) = 0
\]
However, since $p$ is a polynomial of degree 3, $p'$ has degree 2, i.e., $p'(x)$ is a quadratic polynomial which can have at most two roots.
This contradiction implies that a cubic polynomial having four roots cannot exist and thus a cubic polynomial has at most three roots, as claimed.
...
\begin{remark} The fundamental Theorem of Algebra states that a polynomial of degree $n$ can have at most $n$ roots,
	and this result can be proved using a technique called mathematical induction.
\end{remark}
