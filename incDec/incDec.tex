...
%insert desmos interactive to get the feel for positive derivative means increasing function and vis versa.
...
Then $f(x)$ is increasing on the open interval $I$. Similarly, it is also true that
if $f'(x) < 0$ on $I$, then $f(x)$ is decreasing on $I$.
\end{theorem}
...
\end{enumerate}
The last and most relevant fact is due to the chain rule:
                 $$\text{for} x<0,  \frac{d}{dx} \ln(-x) = \frac{1}{-x}\cdot(-1) = \frac{1}{x}. $$
