...
Let $I$ be an open interval and suppose $f$ is differentiable on $I$.
 If $f'(x) > 0$ on $I$, 
...
xticklabels={}, ymin = -1, ytick={}, yticklabels={}, title={$f'(x)>0$  on $I$}]
...
xticklabels={},ymin=-0.1,ymax=1.5, ytick={}, yticklabels={}, title={$f'(x)<0$ }]

\addplot[domain=-0.5:2, color=blue, thick]{0.3+1/2^x} node[pos=.5, below, sloped] {decreasing} node[pos=.5, above, sloped] {$f'(x) < 0$};
...
Determine the intervals on which the function $f(x) = mx + b$ is increasing or decreasing.\\
In this example, the graph of the function is a straight line with slope, $f'(x) = m$.
The line is increasing on $(-\infty, \infty)$ if $m >0$ and decreasing on $(-\infty, \infty)$ if $m<0$.
...
Finally, by the increasing/decreasing theorem, we can conclude that $\ln|x|$ is increasing on the interval $(0, \infty)$,
and decreasing on the interval $(-\infty, 0)$.
...
The derivative of $-e^{2x}$ is $\answer{-2e^{2x}}$.\\
Is $f(x) = -e^{2x}$ increasing or decreasing on the interval $(-\infty, \infty)$?
\begin{multipleChoice}
\choice{increasing}
...
The derivative of $2-3x$ is $\answer{-3}$.\\
Is $f(x) = 2-3x$ increasing or decreasing on the interval $(-\infty, \infty)$?
\begin{multipleChoice}
\choice{increasing}
\choice[correct]{decreasing}
\end{multipleChoice}
