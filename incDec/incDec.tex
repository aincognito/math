...
The following theorem is a direct consequence of the cornerstone, Mean Value Theorem (section 3.3).
...
%or:  Let $I$ be an open interval and suppose $f$ is differentiable on $I$.
 \begin{enumerate}
\item[a)] If $f'(x) > 0$ on $I$, then $f(x)$ is increasing on $I$.
\item[b)]  If $f'(x) < 0$ on $I$, then $f(x)$ is decreasing on $I$.
 \end{enumerate}
\end{theorem}
...
\begin{proof} (Increasing case): Let $x_1$ and $x_2$ be in the interval $I$ with $x_1 < x_2$.  
Since $f$ is differentiable on the closed interval 
$[x_1, x_2]$, it is also continuous there.  
Hence, we can apply the MVT on to $f(x)$ on $[x_1, x_2]$ and conclude that there is a number $c$
between $x_1$ and $x_2$ such that 
\[
f'(c) = \frac{f(x_2) - f(x_1)}{x_2 - x_1}\]
Since $f'(x) > 0$ on the interval $I$ (by hypothesis), we know that $f'(c) >0$.
We also know that $x_2 - x_1 > 0$ since $x_1 < x_2$. Therefore, we must have
$f(x_2) - f(x_1) > 0$ which is the same as $f(x_1) < f(x_2)$.  
We can now see that if $f'(x) > 0$ on $I$ and if $x_1$ and $x_2$ are any 
two values in $I$ with $x_1 < x_2$ then we have $f(x_1) < f(x_2)$.
This means that $f(x)$ is increasing on $I$.\\
