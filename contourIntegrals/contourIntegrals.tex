...
Let $C= C(i, 2)$ be the circle centered at $i$ of radius $2$, traversed in the counter-clockwise direction. Compute \\
...
\[
\left| \int_C f(z) \, dz \right| \leq \int_C \left| f(z) \right| \, dz
\]
\end{proposition}
...
\]
we have an anti-derivative for $\cos z$ for any $z \in \C$. Thus
\[
...
Let $C$ be any contour from $1$ to $1+i$ that does not cross the negative real axis of go through the origin. 
