...
\[
\lim_{t \to 0} \vector{\frac{\sin t}{t}, \frac{\ln(1+t)}{t}, \frac{e^t - t -1}{t^2}}
\]
...
\[
\lim_{t \to 0} \frac{\sin t}{t} = \frac00 \,\overset{\scriptsize\text{L'H}}{=}\, \lim_{t\to 0} \cos t = \cos 0  = 1
\]
\[
\lim_{t \to 0} \frac{\ln(1+t)}{t} = \frac00 \,\overset{\scriptsize\text{L'H}}{=}\, \lim_{t\to 0} \frac{1}{1+t} = 1
\]
\[
\lim_{t \to 0} \frac{e^t - t - 1}{t^2} = \frac00 \,\overset{\scriptsize\text{L'H}}{=}\, \lim_{t\to 0} \frac{e^t - 1}{2t} 
\,\overset{\scriptsize\text{L'H}}{=}\, \frac00 = \frac{e^t}{2}  = \frac12
\]
...
Since continuity is determined componentwise, we can take advantage of our knowledge of ordinary
continuous functions to determine the continuity of a vector-valued function.
...
The following theorem tells us that we can compute the derivative of a vector-valued function componentwise.

\begin{theorem}
If $\vec{r}(t) = \vector{x(t), y(t), z(t)}$ is differentiable, then $\vec{r'}(t)$ is computed via componentwise differentiation, i.e.,
\[
...
\begin{align*}
\vec{r\,'}(t) &= \lim_{h \to 0} \dfrac{\vec{r}(t+h)-\vec{r}(t)}{h}\\
&= \lim_{h \to 0}\dfrac{1}{h}\left[\vector{x(t+h), y(t+h), z(t+h)} - \vector{x(t), y(t), z(t) } \right]\\
  &=\lim_{h \to 0} \vector{\dfrac{x(t+h)-x(t)}{h},  \dfrac{y(t+h)-y(t)}{h},\dfrac{z(t+h)-z(t)}{h}}\\
  &= \vector{x\,'(t), y\,'(t), z\,'(t)}
\end{align*}
...
Find both the equation of the tangent line and the unit tangent vector to the helix
\[
...
Hence, the equation of the tangent line (in vector form) is given by
\begin{align*}
\vector{x, y, z} &= \vec r(a) + t\,\vec r\,'(a)\\
&= \vec r(\pi/2) + t\,\vec r\,'(\pi/2)\\
  &= \vector{-1, 0, \pi/2}  +t\vector{0, -2, 1}
\end{align*}

Next, recall that the unit vector in the same direction as $\vec{v}$ is given by $\vec{u} = \frac{\vec{v}}{|\vec{v}|}$.
...
\begin{center}
\desmosThreeD{hzooegooi5}{800}{600}
\end{center}
...
\begin{center}
\desmosThreeD{gqtm36fy8h}{800}{600}
\end{center}
...
\textbf {Solution:} The tangent vector at $ t = \pi$ is
\[
\vec r\,'(\pi) = \vector{-\sin(\pi), \cos(\pi), 1} = \vector{0, -1, 1}
\]
and the speed of the curve at $t = \pi$ is
\[
