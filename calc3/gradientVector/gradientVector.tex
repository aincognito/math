...
The \textbf{derivatve of} $\mathbf{f}$ \textbf{in the direction of} $\mathbf{\vec u}$ is given by
\[
D_{\vec u} f(x,y) = \lim_{h \to 0} \frac{f(x + ah, y + bh) -f(x,y)}{h}
\]
This derivative gives the \textbf{rate of change of} $\mathbf{f}$ \textbf{in the direction of} $\mathbf{\vec u}$.
\end{definition}
