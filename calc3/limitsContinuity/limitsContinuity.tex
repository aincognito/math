...
\]
All the paths so far yield the same limit.  Now, however, consider a parabolic path toward the origin in which $y = x^2$.
...
\[
\frac{x^3y^2}{x^4 + y^4} \geq -\frac{|x|x^2y^2}{x^4 + y^4} \geq -\frac{|x|(x^4 + y^4)}{x^4 + y^4} = -|x|
\]
...
In each of the following limit laws, the point $(x,y)$ approaches the point $(a,b)$. For ease of readability, 
we abbreviate $\lim_{(x,y) \to (a,b)} f(x,y)$ with $\lim f(x,y)$.\\
