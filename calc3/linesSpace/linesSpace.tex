...
\[
\vec{v} = \arrowvec{PQ} = \vector{x_1-x_0,y_1-y_0,z_1-z_0} = \vector{a,b,c}
\]
...
Here is a video solution to problem 2d:\\
\begin{foldable}
\youtube{71B5E2rpvIY}
\end{foldable}
...
\[
\vec{w} = \arrowvec{PQ} = \vector{3, -1, 2}
\]
Project this vector onto the direction vector of the line, $\vec{v} = \vector{4, -1, 5}$ :
\[
\proj_{\vec{v}}\vec{w} = \frac{\vec{v} \dotp \vec{w}}{\vec{v} \dotp \vec{v}}\vec{v} = \frac{23}{42}\vector{4,-1,5}
\]
...
\begin{image}
\begin{tikzpicture}
\draw[black, ->, very thick, dashed] (0,0) -- (3, 0) node[right]{$\vec{v}$};
\draw[blue!50!black, ->, thick] (0,0) -- (2, 4) node[midway, left]{$\vec{w}$};
\draw[red!90!black, ->, thick] (0,0) --(2,0) node[midway, below]{$\proj_{\vec{v}} \vec{w}$};
...
&=\vector{3, -1, 2} - \frac{23}{42}\vector{4,-1,5} \\
&= \vector{\frac{34}{42},-\frac{19}{42},-\frac{31}{42}}
\end{align*}
and the distance we seek is
\[
|\vec{n}| = \frac{1}{42}\sqrt{ 34^2 +  \left(-19\right)^2 + \left(-31\right)^2} = \frac{\sqrt{1156+361+961}}{42} \approx 1.2
\]
...
\end{hint}
\begin{hint}
Project the vector $\arrowvec{PQ}$ onto the direction vector $\vec v$
\end{hint}
\begin{hint}
The distance is the magnitude of difference of $\arrowvec{PQ}$ and the projection vector
\end{hint}
...
In the desmos 3d interactive below, the blue point is P and the direction vector for the line, $\arrowvec{PQ}$, is the red vector.\\
Use the $t$ slider to move the purple point along the line. 
\begin{center}
\desmosThreeD{xhlksoq0g6}{800}{600}
\end{center}
