...
\draw[->, blue, thick] (0,0) -- (2, 2) node[right]{$\vector{2,2}$};
...
\draw[->, blue, thick] (1.5, 1.5) -- (2.5, 2.5);
\draw[->, blue, thick] (-2.5, -2.5) -- (-1.5,-1.5);
\draw[->, blue, thick] (-0.5, 1.5) -- (0.5, 2.5);
\draw[->, blue, thick]  (-2.5, -0.5)-- (-1.5, 0.5);
\draw[->, blue, thick] (-2.5, 1.5) -- (-1.5, 2.5);
\draw[->, blue, thick] (1.5, -0.5) -- (2.5, 0.5);
\draw[->, blue, thick]  (-0.5, -2.5)-- (0.5,-1.5);
\draw[->, blue, thick] (1.5, -2.5) -- (2.5, -1.5);
%\draw[->] (-1,0)--(0,1);
\draw[->, blue, thick] (-0.5, 1.5) -- (0.5, 2.5);
%\draw[->, blue] (-2,0) -- (-1, 1) ;
%\draw[->, blue] (0,1) -- (1, 2) ;
...
\]
\begin{proof}
Since $|\vec{v}| \neq 0$, the scalar $\displaystyle \frac{1}{|\vec{v}|}$ is a positive real number.  The vector $\vec{u}$ points 
...
Here is a video solution of problems 3a and 3b:\\
\begin{foldable}
\youtube{v8VpOSK3fh8}
\end{foldable}
...
\draw[blue!70!white, ->] (3,0) -- (3, -1) node[below]{weight, $\vec{w} = 20$N};
\draw[blue!70!white, <-] (1, 1) node[left]{friction, $\vec{f} = 8$N} -- (1.5,1);
\draw[blue!70!white, ->] (4.5, 2) -- (6, 3) node[right]{rope force,   $\vec{r} = 24$N};
...
The force of the weight of the box has a magnitude of $20$ and points in the direction of the negative $y$-axis, i.e., $\vec{w} = \vector{0,-20}$.\\
The friction force has a magnitude of $8$ and points in the direction of the negative $x$-axis, i.e., $\vec{f} = \vector{-8,0}$.\\
...
%Assuming that the vertical component of the rope force, $\vec{r}$, is not enough to overcome the weight of the box (and thus send it airborne), 
Lastly, the floor will exert an upward normal force on the box 
(the box is assumed to be on the floor)
This force will have the form $\vec{n} = \vector{0, s}$ with $s \geq 0$. 
The value of $s$  will be determined by the fact that the net vertical component of all forces acting on the box is zero. 
