...
Describe the level curves of the function $z = f(x, y) = \sqrt{1-x^2 - y^2}$ and sketch a contour map consisting of at least 3 of them.\\.\\
...
\node at (3.8, -0.3) {$1$};
\node at (-.3, 3.8) {$1$};
\node at (0, -5.5) {A contour map consisting of three level curves of $f(x, y)= \sqrt{1 - x^2 - y^2}$};
\end{tikzpicture}
\end{image}
