...
Then $f(x,y)$ attains an absolute maximum and absolute minimum value on $S$.
\end{theorem}
...
The previous section introduced the process of finding critical points of a function—points where the gradient is zero.
This section shifts focus to boundary points, where maximum and minimum values can also occur.
...
Hence, either $f\,'(t) = 0$ at this point (or $f\,'(t)$ is undefined at this point). From the chain rule
\[
0 = f\,'(t) = f_x(x, y) x'(t) + f_y(x,y) y'(t) = \grad f(x,y) \cdot \vector{x'(t), y'(t)}
\]
This means that at the absolute max or absolute min, the gradient of $f$ is orthogonal to the boundary curve.
...
Thus, at the maximum and minimum values of $f(x,y)$ along the boundary curve $g(x,y) = 0$ of the set $S$, 
\[
...
We now turn to the problem of finding the \textbf{absolute maximum} and \textbf{absolute minimum} values of a function of 
several variables on a \textbf{closed and bounded} subset of its domain. To say that a set is \textbf{bounded} means that it is 
contained within some disk of finite radius. That is, there exists a disk centered at the origin (or any point) such that the entire set lies within it.
To say that a set is \textbf{closed} means that it contains all of its \textbf{boundary points}. A point is a 
boundary point of a set $S \subseteq \mathbb{R}^2$ if every disk centered at that point contains points both in $S$ and not in $S$. 
Points in $S$ that are not boundary points are called \textbf{interior points}.

As an example, consider the \textbf{unit circle}, defined by the equation $x^2 + y^2 = 1$. This set is both closed and bounded. Moreover, every 
point on the unit circle is a boundary point.

The \textbf{unit disk} (on the other hand) is the set of all points $(x, y) \in \mathbb{R}^2$ such that $x^2 + y^2 < 1$. It is also bounded set, 
but it is \textbf{not} closed. This is because it does not include its boundary, which is the unit circle. 
Every point within the unit disk is classified as an interior point.

The \textbf{closed unit disk} is the union of the unit disk and the unit circle, and it is described by the inequality $x^2 + y^2 \leq 1$. 
This set is both closed and bounded. In problems of optimization, such sets are particularly important because continuous functions always 
attain maximum and minimum values on closed and bounded sets. 
