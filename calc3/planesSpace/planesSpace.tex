...
\]
\[
\text{or} \quad a(x-x_0) + b(y-y_0) + c(z-z_0) = 0 %\quad \text{(Canonical linear form)}
\]
Using $d = ax_0 + by_0 + cz_0$,  we can write the more succinct
\[
\vec{n} \dotp \vector{x, y, z} = d %\quad \text{(Vector form)}
\]
\[
\text{or} \quad \quad ax+by+cz = d \quad\text{(Standard form)}
\]
\end{proposition}
...
The normal vector to both planes is $\vec{n} = \vector{1, 2, -1}$. 
...
Find the distance between the planes $P_1: 2x -5y + 3z = 6$ and $P_2: 2x-5y+3z = -2$.\\
\end{problem}
...
Find the distance between the skew lines $\vector{1, 2, 3} + t\vector{4, 5, 6}$
and  $\vector{3, 2, 1} + t\vector{6, 5, 4}$.\\
\begin{hint}
...
\end{hint}
\begin{hint}
Write the equations of the parallel planes using oen point from each plane and the normal vector
\end{hint}
\end{problem}
\end{document}
