...
that computing definite integrals can be done via anti-differentiation!


\begin{theorem}[Fundamental Theorem of Calculus]
If $f(x)$ is continuous on the interval $[a, b]$ and if $F(x)$ is an 
antiderivative of $f(x)$, i.e., $F'(x) = f(x)$, then
\[
...
The proof of the Fundamental Theorem of Calculus can be obtained by applying the Mean Value Theorem 
to $F(x)$ on each of the sub-intervals $[x_{i-1}, x_i]$
...
two main steps: first, find the antiderivative
...
In the first step, the function $F(x)$ is introduced and in the second step, the difference 
of the values $F(b)$ and $F(a)$ is computed.
...
\end{hint}
    \begin{hint}
      Use the power rule to find an antiderivative of $x^3$
    \end{hint}
...
Since the function $\cos(x)$ is non-negative on the interval $[0, \frac{\pi}{2}]$, the exact area can be 
...
\begin{image}
\begin{tikzpicture}
\begin{axis}[axis x line=  bottom, axis y line = left,xtick={0, 0.785, 1.57}, 
  xticklabels={$0$, $\frac{\pi}{4}$,$\frac{\pi}{2}$}, ytick={0, .5, 1}, title={The area under $f(x)=\cos(x)$ on $[0,\pi/2]$}]
...
\begin{image}
\begin{tikzpicture}
\begin{axis}[axis x line=  bottom, axis y line = left,xtick={0, .5, 1}, ytick={0, .5, 1,1.5, 2, 2.5}, 
  title={The area under $f(x)=e^x$ on $[0,1]$}]
\addplot[name path = A, domain=0:1, 
...
The area is $\answer{60}$.
 ...
 \[
  ...
  \end{hint}
    \begin{hint}
      An antiderivative of $x^3$ is $x^4 /4$
    \end{hint}
    \begin{hint}
      The Fundamental Theorem says:
      \[
...
\begin{image}
\begin{tikzpicture}
\begin{axis}[axis x line=  bottom, axis y line = left, xtick={0,1,2,2.72, 3}, xticklabels ={0, 1, 2, $e$, 3}, 
  ytick={0, .5, 1}, title={The Area Under $f(x)=\dfrac{1}{x}$ on $[1,e]$}]
...
The area is $\answer{\ln(5/2)}$.
 \begin{hint}
  ...
  \end{hint}
    \begin{hint}
      An antiderivative of $1/x$ is $\ln(x)$
    \end{hint}
    \begin{hint}
      The Fundamental Theorem says:
      \[
...
The area is $\answer{10}$.
 ...
 \end{hint}
    \begin{hint}
      An antiderivative of $4x-3$ is $2x^2 - 3x$
    \end{hint}
    \begin{hint}
      The Fundamental Theorem says:
      \[
...
Since the function $\cos(x)$ is non-negative on the interval $[0, \frac{\pi}{4}]$, the exact area can be 
expressed as the definite integral
...
\end{hint}  
    \begin{hint}
      An antiderivative of $\csc^2(x)$ is $-\cot(x)$
    \end{hint}
...
Since the function $\frac{1}{1+x^2}$ is non-negative on the interval $[0, 1]$, the exact area can be expressed as the definite 
integral
...
\begin{image}
\begin{tikzpicture}
\begin{axis}[axis x line=  bottom, axis y line = center,xtick={-3, -2, -1, 0, 1, 2, 3}, ytick={0, .5, 1,1.5, 2, 2.5}, 
  title={The Area Under $f(x)=\dfrac{1}{1+x^2}$ on $[0,1]$}]
\addplot[name path = A, domain=0:1, 
...
Find the exact area under the graph of $y = \dfrac{1}{\sqrt{1-x^2}}$ from $x = 0$ to $x = 1/2$.\\
The area is $\answer{\pi/2}$.
 \begin{hint}
  \[
  \text{Area} =  \int_0^{1/2} \frac{1}{\sqrt{1-x^2}} \ dx
  \]
 \end{hint} 
    \begin{hint}
      An antiderivative of $\frac{1}{\sqrt{1-x^2}}$ is $\sin^{-1}(x)$
    \end{hint}
    ...
    \end{hint}    
		\begin{hint}
      $\sin^{-1}(0) = 0$ and $\sin^{-1}(1) = \frac{\pi}{6}$ 
    \end{hint}
		
		
\end{problem}

Here is a video solution to problem 7.
\begin{foldable}
\youtube{Z7SoAsa1Eow} 
\end{foldable}
...
Since the function $x^3$ is negative (or zero) on the interval $[-1, 0]$ , the exact area above the curve is the 
negative of the definite integral 
\[\int_{-1}^0 x^3 \ dx.\]
We can compute the value of this definite integral using the Fundamental Theorem of Calculus as follows:
...
Since the function $x^2 - 2 \leq 0$  on the interval $[0,1]$ , the exact area above the curve \textit{is the negative of} the 
definite integral 
\[\int_0^1 (x^2 -2) \ dx.\]
We can compute the value of this definite integral using the Fundamental Theorem of Calculus as follows:
...
Since the function $-\sin(x) \leq 0$ on the interval $[0, \pi]$ , the exact area above the curve 
is \textit{the negative of} the definite integral 
\[\int_{0}^\pi -\sin(x) \ dx.\]
We can compute the value of this definite integral using the Fundamental Theorem of Calculus as follows:
...
Here is a video solution to problem 10b.
\begin{foldable}
\youtube{Gh-q_L4U6zQ} 
\end{foldable}
...
Here is a video solution to problems 11a, d and e.
\begin{foldable}
\youtube{UidDJBP47tA} 
\end{foldable}
