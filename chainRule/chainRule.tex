...
The function $\sqrt{x^2 + 9}$ is a composition with $g(x) = x^2 + 9$ inside and $f(x) = \sqrt{x}$ outside.
\[g(x) = x^2 + 9   \quad \text{and} \quad  f(x) = \sqrt{x}.\]


To find the derivative of $\sqrt{x^2 + 9}$ we need to use the chain rule.
We need both $f'(g(x))$ and $g'(x)$. The second is simpler so we start there. We have, $g'(x) = 2x$.
