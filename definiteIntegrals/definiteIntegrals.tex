...
If the graph of the function $f(x)$ is a line or semicircle, then we may be able to compute the definite integral 
...
\addplot[name path = A, domain=1:4, samples=100, color=black]{4}node[below, midway]{$y = 4$} ;
...
\addplot[name path = A, domain=0:3, 
    samples=100, color=black]{2*x} node[midway, above left]{$y = 2x$};
...
\addplot[name path = A, domain=0:2, 
    samples=100, color=black]{x+1} node[midway, above left]{$y = x+1$};
...
\[y = \pm \sqrt{r^2 - x^2}.\]
The graph of $y = \sqrt{r^2 -x^2}$ is the upper semicircle and the
graph of $y = -\sqrt{r^2 -x^2}$ is the lower semicircle.
...
%\[y = \pm \sqrt{r^2 - x^2}.\]
%Taking the positive square root gives the equation of the upper semi-circle and 
%taking the negative square root gives the equation of the lower semi-circle.
The graph of the function $f(x) = \sqrt{4-x^2}$ is an upper semicircle with center at the origin and radius $2$, shown below. 
The definite integral in question represents the area of this semicircle, which is given by
\[
A = \frac12 \pi r^2 = \frac12 \pi (2^2) = 2\pi.
\]
Therefore, the definite integral is $2\pi$, i.e.,
...
enlarge y limits = .15, ymin = -0.37,
title={The area under $f(x)=\sqrt{4-x^2}$ on $[-2, 2]$}]
\addplot[name path = A, domain=-2:2, 
    samples=100, color=black]{(4-x^2)^(1/2)};
\addplot[->, thin, samples = 100, color=black] coordinates {(0,2.2) (0,-0.76)};
\addplot[<-, name path = B, domain=-2.1:2.1, samples=100, color=black]{0};
\addplot[blue!10] fill between[of=A and B];
