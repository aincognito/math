...
\subsection{Higher Order Derivatives of Sine and Cosine}
...
Find the first four derivatives of $f(x) = \sin x$, and then find the 51st derivative, $f^{(51)}(x)$.\\
\begin{align*}
f'(x) &= \cos x\\
f''(x) &= -\sin x\\
f'''(x) &= -\cos x\\
f^{(4)}(x) &= \sin x
\end{align*}  
Note that the fourth derivative is the same as the original function.  
This pattern will continue for all higher derivatives of $f(x) = \sin x$.  \\
We can use this pattern to find the 51st derivative.  Since $51 = 4(12) + 3$, we have
\[
f^{(51)}(x) = f^{(4(12)+3)}(x) = f^{(3)}(x) = -\cos x.
\]
...
Find the first four derivatives of $f(x) = \cos x$, and then find the 38th derivative, $f^{(38)}(x)$.
\begin{align}
f'(x) &= \answer{-\sin x}\\
f''(x) &= \answer{-\cos x}\\
f'''(x) &= \answer{\sin x}\\
