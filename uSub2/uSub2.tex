...
Now, the original integral which was in terms of the variable $x$ and the differential $dx$ can be rewritten in terms of the new variable $u$ and its differential $du$.  
Substituting $u$ for $x^2 + 1$ and $du$ for $2x \  dx$, the original integral becomes 
...
\[\int e^{x/2} \ dx.\]
Let $u = \frac{x}{2}$, then $du = \tfrac12 \ dx$.
Since $2 \cdot \frac12 = 1$, the integral can be written as 
\[\int e^{x/2} \ dx =  2\int \tfrac{1}{2}e^{x/2}   \   dx =  2 \int e^u \ du.\]
The last integral can be computed as 
