...
If
\[\lim_{x\to a} g_1(x) = \lim_{x\to a} g_2(x) = L,
\]
...
We conclude this section by using the Squeeze Theorem to find a special limit, $\lim_{\theta \to 0} \frac{\sin(\theta)}{\theta}$.

\begin{image}
\begin{tikzpicture}
\draw[thin](0,0) -- (4,0) -- (4, 4) -- cycle;
\draw[thin] (2.828, 0) -- (2.828, 2.828);
\draw[thin] (4,0) arc (0: 45:4);
\node[right] at (0.2,0.2) {$\theta$};
\node[below] at (1.5,0) {$\cos(\theta)$};
\node[left] at (2.8,1.2) {$\sin(\theta)$};
\node[right] at (4.1,2) {$\tan(\theta)$};
\node[above] at (1.3,1.4) {$1$};
...
Consider the figure above.  It consists of a small triangle, a sector of a circle of radius $1$ and a large triangle.\\
...
\[
\lim_{\theta \to 0^+} \cos(\theta) = \cos(0) = 1 \text{   and   } \lim_{\theta \to 0^+} \frac{1}{\cos(\theta)} = \frac{1}{\cos(0)} = 1. 
\]
...
To compute the left-hand limit, we note that $\sin(-\theta) = -\sin(\theta)$ for any angle $\theta$.
Therefore,
\[
