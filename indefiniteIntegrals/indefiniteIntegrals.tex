...
\frac{d}{dx} x^2 = 2x,
\]
and this leads us to the answer:
\[
f(x) = x^2.
\]
...
\]
Thus, the answer to the anti-differentiation problem is actually not just a single function but a family of functions 
each pair of which differing by a constant.
...
\]
But let us not forget the lesson of the previous example: there is more than one answer.  The answer is a family of functions,
each pair of which differing by a constant. So a complete solution set to our problem is
\[
s(t) = 40t - 16t^2 + C.
\]
Note that the constant in this equation represents the position of the object at time $t = 0$.
We denote this particular constant as $s_0$ (rather than $C$), the \textbf{initial position} of the object, and we can write our answer as 
$s(t) = s_0 + 40t - 16t^2$.
...
Here is a video solution to problems 4a, 4b and example 5.
\begin{foldable}
\youtube{i52dx72vREU} 
\end{foldable}
...
Here is a video of several examples of the power rule for indefinite integrals and the special case of $n=-1$.
\begin{foldable}
\youtube{lLNmFEWpJtY} 
\end{foldable}
