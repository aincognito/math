...
\subsection{Higher Order Derivatives of the Natural Exponential and Logarithm}
...
Find the first five derivatives of $f(x) = \ln x$, and then find a formula for $f^{(n)}(x)$.\\
\begin{align*}
f'(x) &= \frac{1}{x} = x^{-1}\\
f''(x) &= -x^{-2} = -\frac{1}{x^2}\\
f'''(x) &= 2x^{-3} = \frac{2}{x^3}\\
...
\end{align*}  
Noting the pattern, we can write a formula for the $n$th derivative of $f(x) = \ln x$:
\[
f^{(n)}(x) = (-1)^{n+1}(n-1)!x^{-n} = (-1)^{n+1}\frac{(n-1)!}{x^n}.
\]
\end{example}
...
Find the first three derivatives of $f(x) = 3e^x$, and then find a formula for $f^{(n)}(x)$.\\
\begin{align}
f'(x) &= \answer{3e^x}\\
f''(x) &= \answer{3e^x}\\
f'''(x) &= \answer{3e^x}\\
