...
\outcome{Use average velocity to guess instantaneous velocity}
...
The Greek letter $\Delta$ (delta) represents ``change in", so $\Delta h$ is the change in 
height and $\Delta t$ is the change in time. 
To represent the entire 100-foot fall of the object, 
...
The initial height of the object is $\answer{400}$ ft\\
The object hits the ground after $\answer{5}$ sec\\
The average velocity during the fall is $v_{ave}[0, 5] = \answer{-100}$ ft/sec\\
...
At times $t = 2$ and $t= 4$, the corresponding heights are:
\[
h(2) = \answer{236}\, \text{feet}, \; \text{and} \; h(4) = \answer{44} \,\text{feet}
\]
The average velocity of the object over the time interval $[2,4]$ is: 
\[
v_{ave}[2,4] = \answer{-96}\, \text{ft/sec}
\]
The negative sign  indicates that the object is \wordChoice{\choice{rising}
\choice[correct]{falling}}
\end{problem}
...
In the last example, with $s(t) = 100 - 16t^2$, we found that $s(0) = 100$ ft and $s(5/2) = 0$ ft. If we plot the points
$(0, 100)$ and $(5/2, 0)$ then the slope of the segment connecting them is the average velocity of the falling object from time
$t = 0$ seconds to time $t = 5/2$ seconds, as shown in the figure below.
...
\filldraw (0,3) circle(.05) node[left] {$100$};
\filldraw (2,0) circle(.05) node[below] {$5/2$};
\draw[ultra thick, blue] (0,3) --(2,0) node[midway,right] {$m = -40$ft/sec};

\node at (2,-1) {The slope of the blue line is the average velocity};
\node at (2, -1.5) {of the object from $t = 0$ to $t = 5/2$};
...
\begin{array}{ c | c  }
   \text{interval}& v_{ave} \\ 
	\hline
	[2.9, 3] & \answer{-94.4} \text{ft/sec} \\
	\hline
	[2.99, 3] & \answer{-95.84} \text{ft/sec} \\
	\hline
	[3, 3.1] & \answer{-97.6} \text{ft/sec} \\
	\hline
	[3, 3.01] & \answer{-96.16} \text{ft/sec}
\end{array}
\]
...
\begin{array}{ c | c  }
   \text{interval}& v_{ave} \\ 
	\hline
	[0.9, 1] & \answer{19.6} \text{ft/sec} \\
	\hline
	[0.99, 1] & \answer{18.16} \text{ft/sec} \\
	\hline
	[1, 1.1] & \answer{16.4} \text{ft/sec} \\
	\hline
	[1, 1.01] & \answer{17.84} \text{ft/sec}
\end{array}
\]
...
We saw that the average velocity of a falling object can be represented by the slope of a \textbf{secant line}. 
The instantaneous velocity can also be represented by the slope of a \textbf{tangent line}.

The function in the graph below represents the position function for a falling object.  
