...
\draw[thin, dashed, blue!80, ->] (0,0) -- (1, 1) node[midway, left] {${\bf u}$};
\draw[blue, ->, thick] (1,1) -- (3,2) node[midway, blue, above] {$\bf{v}$};
\draw[mark=*,mark size=1pt, blue!80] plot coordinates {(0, 0)} node[below left]{$0$} ;
\draw[mark=*,mark size=1pt, blue] plot coordinates {(1, 1)} ;
\draw[mark=*,mark size=1pt, blue] plot coordinates {(3, 2)} ;
\draw[mark=*,mark size=1pt,red] plot coordinates {(1.5, 1.25)} node[below right] {$\gamma(t) = {\bf u}+ t{\bf v}$} ;
\node at (1.5, -0.8){\Large The segment from ${\bf u} \;\; \mbox{to}\;\; {\bf u}+{\bf v}$};
\end{tikzpicture}
\end{image}

The vector ${\bf u} = (x,y,0)$ and the vector ${\bf v} = (-x, -y, 1)$. We wish to find $t$ so that the point
\[
...
\[
\varphi(x+iy) = \varphi(x,y) = (x(1-t),y(1-t),t)  = \left( \frac{x}{1+x^2 +y^2}, \frac{y}{1+x^2 +y^2}, \frac{x^2 + y^2}{1+x^2+y^2} \right)
\]
