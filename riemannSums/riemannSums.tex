...
xticklabels={$a$, $b$}, title={The Area Problem}] 
...
To solve this problem, we begin by approximating the area under the curve using rectangles. 
...
%axes
\addplot[<->] coordinates {(-1.5,-.5) (-1.5, 2)};
\addplot[<-] coordinates {(-2.02,0) (1.8, 0)};
...
Find the Riemann Sum for the curve $y = e^{-x}$ over the interval $[-1,1]$ using $n=4$ rectangles 
and left endpoints as the sample points.\\
First, we compute $\Delta x$ and the sample points $x_i^*$:
\[
\Delta x = \frac{b-a}{n} =\frac{1 - (-1)}{4} = \frac{1}{2}
\]
The sub-intervals are $[-1, -\tfrac12], [-\tfrac12, 0], [0, \tfrac12]$ and $[\tfrac12, 1]$. 
Since we are using left endpoints as the sample points, we have
\[
x_1^* = -1,\;  x_2^* = -\frac12,\; x_3^* = 0,\; \mbox{and}\; x_4^* = \frac12
\]
Now, with $f(x) = e^{-x}$, the value of the Riemann sum is 
\begin{align*}
\sum_{i=1}^4 f(x_i^*)\Delta x &= f(x_1^*)\Delta x + f(x_2^*)\Delta x + f(x_3^*)\Delta x + f(x_4^*)\Delta x\\
&= f(-1)\cdot \left(\tfrac12\right) + f(-\tfrac12) \cdot \left(\tfrac12\right) +f(0) \cdot \left(\tfrac12\right) +f(\tfrac12) \cdot \left(\tfrac12\right)\\
&= \left(\tfrac12\right) \left(e^{1}  + e^{1/2}  +  e^0 + e^{-1/2} \right) \approx 2.987
\end{align*}

\begin{image}
\begin{tikzpicture}
\begin{axis}[axis x line=  bottom, axis y line = center, xtick={-1, -1/2, 0, 1/2, 1},  
ytick={0, 1, 2}, title={Riemann Sum for $y=e^{-x} $ on $[-1,1]$ with $n=4$ using Left Endpoints}]
...
Find the Riemann Sum for the curve $y = x^2 + 1$ over the interval $[0,1]$ using $n=5$ rectangles 
and left endpoints as the sample points.\\

The width of each rectangle is $\Delta x = \answer{1/5}$\\
The sample points are $x_1^* = \answer{0}$, $x_2^* = \answer{1/5}$, 
$x_3^* = \answer{2/5}$, $x_4^* = \answer{3/5}$ and $x_5^* = \answer{4/5}$.\\
The Riemann Sum is $\answer{1.24}$.

\begin{image}
\begin{tikzpicture}
\begin{axis}[axis x line=  bottom, axis y line = left, xtick={0, 1/5, 2/5, 3/5, 4/5, 1},  
ytick={0, 1, 2}, title={Riemann Sum for $y=x^2 + 1 $ on $[0,1]$ with $n=5$ using Left Endpoints}]
\addplot[domain=0:1, 
...
Here is a video solution to problem 4a.
\begin{foldable}
\youtube{CLyRkHn-kI4} 
\end{foldable}

\begin{problem} (problem 4b)
Find the Riemann Sum for the curve $y = x^3 $ over the interval $[1,3]$ using $n=4$ rectangles 
and left endpoints as the sample points.\\
The width of each rectangle is $\Delta x = \answer{1/2}$\\
The sample points are $x_1^* = \answer{1}$, $x_2^* = \answer{3/2}$, 
$x_3^* = \answer{2}$ and $x_4^* = \answer{5/2}$.\\
The Riemann Sum is $\answer{14}$.
\end{problem}
...
\]
The sub-intervals are $[-1, -\tfrac12], [-\tfrac12, 0], [0, \tfrac12]$ and $[\tfrac12, 1]$. 
Since we are using right endpoints as the sample points, we have
\[
x_1^* = -\frac12,\;  x_2^* = 0,\; x_3^* = \frac12,\; \mbox{and}\; x_4^* = 1
\]
Now, with $f(x) = e^{-x}$, the value of the Riemann sum is 
\begin{align*}
\sum_{i=1}^4 f(x_i^*)\Delta x &= f(x_1^*)\Delta x + f(x_2^*)\Delta x + f(x_3^*)\Delta x + f(x_4^*)\Delta x\\
&= f(-\tfrac12) \cdot \left(\tfrac12\right) + f(0) \cdot \left(\tfrac12\right) +f(\tfrac12) \cdot \left(\tfrac12\right) +f(1) \cdot \left(\tfrac12\right)\\
&= \left(\tfrac12\right) \left(e^{1/2} + e^0 + e^{-1/2} + e^{-1} \right) \approx 1.812
\end{align*}

\begin{image}
\begin{tikzpicture}
\begin{axis}[axis x line=  bottom, axis y line = center, xtick={-1, -1/2, 0, 1/2, 1},  
ytick={0, 1, 2}, title={Riemann Sum for $y=e^{-x} $ on $[-1,1]$ with $n=4$ using Right Endpoints}]
...
Find the Riemann Sum for the curve $y = x^2 + 1$ over the interval $[0,1]$ using 5 rectangles 
and right endpoints as the sample points.\\
The width of each rectangle is $\Delta x = \answer{1/5}$\\
The sample points are $x_1^* = \answer{1/5}$, $x_2^* = \answer{2/5}$, 
$x_3^* = \answer{3/5}$, $x_4^* = \answer{4/5}$ and $x_5^* = \answer{1}$.\\
The Riemann Sum is $\answer{1.44}$.
\begin{image}
\begin{tikzpicture}
\begin{axis}[axis x line=  bottom, axis y line = left, xtick={0, 1/5,2/5,3/5,4/5,1},  
ytick={0, 1, 2}, title={Riemann Sum for $y=x^2 + 1 $ on $[0,1]$ with $n=5$ using Right Endpoints}]
\addplot[domain=0:1, 
    samples=100, color=black]{x^2 + 1};
\addplot[smooth,mark=*,black] plot coordinates {(0,0)};
...
and right endpoints as the sample points.\\
The width of each rectangle is $\Delta x = \answer{1/2}$\\
The sample points are $x_1^* = \answer{3/2}$, $x_2^* = \answer{2}$, 
$x_3^* = \answer{5/2}$ and $x_4^* = \answer{3}$.\\
The Riemann Sum is $\answer{27}$.
\end{problem}
