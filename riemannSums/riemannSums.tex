...
\]
The sub-intervals are $[-1, -0.5], [-0.5, 0], [0, 0.5]$ and $[0.5, 1]$. 
Since we are using left endpoints as the sample points, we have
\[
x_1^* = -1,\;  x_2^* = -0.5,\; x_3^* = 0,\; \mbox{and}\; x_4^* = 0.5
\]
...
&= e^{1} (0.5) + e^{0.5} (0.5) + e^0 (0.5) + e^{-0.5} (0.5) \\
&\approx 2.987
\end{align*}
...
\[
x_1^* = -0.5,\;  x_2^* = 0,\; x_3^* = 0.5,\; \mbox{and}\; x_4^* = 1
\]
Now, with $f(x) = e^{-x}$, the value of the Riemann sum is 
\begin{align*}
\sum_{i=1}^4 f(x_i*)\Delta x &= f(x_1^*)\Delta x + f(x_2^*)\Delta x + f(x_3^*)\Delta x + f(x_4^*)\Delta x\\
&= f(-0.5) \Delta x + f(0) \Delta x +f(0.5) \Delta x +f(1) \Delta x\\
&= e^{0.5} (0.5) + e^0 (0.5) + e^{-0.5} (0.5) +  e^{-1} (0.5) \\
&\approx 1.812
\end{align*}
