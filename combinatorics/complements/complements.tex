...
Given a universal set $U$ and a subset, $S \subset U$, we dfine the complement of $S$, denoted, $S^c$ by
\[
S^c = \{x \in U | x \notin S \}
\]
\end{definition}
...
The number of elements in $S$ is the number of elements in $U$ minus the number of elemetns in $S^c$:
\[
|S| = |U| - |S^c|
\]
\end{proposition}
...
|S^c| = 2 + 1 = 3
\]
Note that $S^c$ was partitioned into two substes here.
...
\]
possibilities. Thus, the Rule for Complements tells us tha tthe number of passwords which contain at least one
special symbol is
\[
