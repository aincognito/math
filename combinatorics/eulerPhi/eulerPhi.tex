...
\outcome{Introduce the Euler $phi$-function}
...
\begin{proposition}[Euler $\phi$-function]
...
The next proposition deals with an important formula in number theory. 
The Euler $\phi$-function, $\phi(n)$, gives the 
number of natural numbers less than $n$ which are relatively prime to $n$. 
For example $\phi(10) = 4$ since the numbers 1, 3, 7 and 9 are less than 10 and relatively prime to 10,
and for any prime number $p$, $\phi(p) = p-1$ since the numbers 1, 2, 3, ..., $(p-1)$ are less 
than $p$ and relatively prime to $p$. To state the proposition, we need the Fundamental Theorem of 
Arithmetic, which says that any natural number greater than 1 has a (unique) prime 
factorization, i.e., if $n >1$ then there exists prime numbers
$p_1, p_2, ...,p_l$ and positive integers $\alpha_1, \alpha_2, ..., \alpha_r$ such that
\[
n = p_1^{\alpha_1}p_2^{\alpha_2} \cdots p_r^{\alpha_l}
\]
