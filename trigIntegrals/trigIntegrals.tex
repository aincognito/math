...
\int \sec^5(x) \tan(x) \, dx.
\]
With an odd power of tangent, we prepare the substitution $u = \sec(x), \; du = \sec(x)\tan(x) \, dx$ by rewritng the integral as
\[
\int \sec^5(x) \tan(x) \, dx = \int \sec^4(x) \sec(x) \tan(x) \, dx.
\]
Next we perfrom the substitution:
\begin{align*}
...
Finally
\[
\int \sec^3(x) \tan(x) \, dx = \int \answer{1/3 \sec^3(x)} + C
\]
...
Substituting gives
\[
\int \tan^5(x) \sec^7(x)\;dx = \int \answer{(u^4 -2u^2 + 1) u^7} \; du
\]

Finally
\[
\int \tan^5(x) \sec^7(x)\;dx =  \answer{\frac{1}{12} \sec^{12}(x) + \frac15 \sec^{10}(x) -\frac18 \sec^8(x)} + C.
\]
\end{problem}
