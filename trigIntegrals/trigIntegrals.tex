...
Compute the indefinite integral:
\[
\int  \sec^6(3x)\;dx = \answer{\frac13 \tan(3x) + \frac19 \tan^3(3x)} + C.
\]
...
Next, let $u = \answer{\tan(3x)}$ so that $du = \answer{3\sec^2(6x) dx}$.\\
...
Finally 
\[
\int \sec^6(3x) \; dx= \answer{\frac13 \tan(6x) + \frac29 \tan^3(6x) + \frac{1}{15}\tan^5(6x)} + C.
\]
