...
&= \int \tan^4(x) \sec^2(x) \; du & \text{(cancelling)}\\
&= \int u^4 \sec^2(x) \; du & \text{(use Pythagorean identity)}\\
&= \int u^4\left[1 + \tan^2(x)\right] \; du \\
&= \int u^4(1+u^2) \; du & \text{(distribute $u^4$)}\\
&= \int (u^4 + u^6) \; du & \text{(now integrate)}\\
&=  \frac{u^5}{5} + \frac{u^7}{7} + C  \\
&= \frac15\tan^5(x)  + \frac17 \tan^7(x) + C & \text{(back substituting)}
\end{align*}
\end{example}
...
the power is odd since if $n$ is odd, $n-2$ is also meaning that if we repeat the formula enough times, 
