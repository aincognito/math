...
the complex logarithm is multivalued.  For a given $w \in \C$, $z=\log w$ gives all of the values of $z$ so that $e^{z} = w$. To define the complex log, 
consider a complex number $w_0$ in the image of $f(z) = e^z$, so that $w_0 = e^{x_0} \cis y_0$.  Then the modulus of $w_0$ is $e^{x_0}$ and an argument of $w_0$ is $y_0$.
