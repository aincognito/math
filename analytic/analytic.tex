...
\begin{example}
Recall from section 3.2 that the function $f(z) = \overline{z}^2$ is differentiable at $z=0$ and nowhere else. Thus,
$f(z) = \overline{z}^2$ is not differentiable on any disk of positive radius. Hence, $f(z) = \overline{z}^2$ is nowhere analytic.
\end{example}

\begin{example}
The function $f(z) = e^z$ is differentiable at every point in $\C$ and hence it is analytic everywhere in $\C$.
\end{example}
...
f'(z_0) &= \lim_{h \to 0 \atop h \in \R} \frac{f(z_0 + h) -f(z_0)}{h} \\[10pt]
        &= \lim_{h \to 0} \frac{\left[u(x_0 +h, y_0) + iv(x_0 +h, y_0)\right] - \left[u(x_0,y_0)+iv(x_0 +h, y_0)\right]}{h} \\[10pt]
