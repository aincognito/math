...
%add problem where students must first draw the peicewise graph then find the limit, a la Stewart, section 2.2 #11
%for example f(x) = x+1 if x < -2 ; x^2 if -2 < x < 2 and 2-x if x > 2
...
the $y$-values on the graph are very close to $2$. And as the $x$-value moves toward 3, the corresponding $y$-value on the graph moves toward 2. 
We can express this using limits as 
...
Here is a problem that encompasses all the ideas in this section.
...
construct the graph of the function.  Many answers are possible, but if your answer is correct, then your graph will have all the properties listed.
...
Here is a video solution to problem 8:
\youtube{nd824Q5F9AM}
