...
\Delta x = \frac{b-a}{n}
\]
Next, we choose a sample point in each subinterval. We denote the sample point in the $i$-th subinterval by $x_i^*$ where $i = 1, 2, \dots , n$.
Then, we make a cross-section of the solid perpendicular to the $x$-axis at each sample point, $x_i^*$ and denote the area of this cross-section by $A(x_i*)$.
...
If we multiply the area of the cross-section, by the width of the sub-interval, 
we get the volume of an object that is roughly the volume of the solid over the sub-interval:
\[
V(x_i^*) = A(x_i^*) \Delta x
\]
