...
xticklabels={$a$, $b$}, title={Area Between Curves}] 
...
\node at (axis cs: .75,1){f(x)};
\node at (axis cs: -.5,.65){g(x)};
...
xticklabels={$1$, $2$}, title={Area between the parabola $y=5-x^2$ and the line $y=x-4$ 
from $x = 1$ to $x=2$}]
...
\node at (axis cs: 0,-6.6){$\displaystyle{A(x)=\int_{a}^{b} \left[f(x)-g(x)\right]\; dx=\frac{13}{6}}$};
