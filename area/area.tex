...
\addplot[thick, dashed, blue] coordinates {(-1,0) (-1, 1)};
\addplot[thick, dashed, blue] coordinates {(1,0) (1, 1)};
...
\addplot[thick, dashed, blue] coordinates {(-1,1.2) (-1, 2)};
\addplot[thick, dashed, blue] coordinates {(1,1.2) (1, 2)};

\addplot[<->] coordinates {(-1.5,-.6) (-1.5, 2.5)};
\addplot[<->] coordinates {(-1.7,0) (1.5, 0)};
\addplot[white] coordinates {(-1.5, -0.7) (-1.5, -1.4)};
...
\node at (axis cs: 0,-1.4){$=\int_a^b \left[f(x)-g(x)\right] \; dx$};
\end{axis}
\end{tikzpicture}
\end{image}
...
The area of the region between $f(x)$ and $g(x)$ is the same as the area
between $f(x) + k$ and $g(x) + k$ for any constant $k$, i.e., shifting the region does not change its area.
If $k$ is large enough then both $f(x) + k$ and $g(x) + k$ will be positive. The result follows by noting that
$(f(x) + k) - (g(x) + k) = f(x) - g(x)$.
\end{remark}
...
\begin{image}
\begin{tikzpicture}
\begin{axis}[scale = 2, axis x line=  none, axis y line = none, xtick={.9, 2.1}, 
...
\node at (axis cs: 0.4,-4.5){$\displaystyle{A=\int_{-1}^{2} \left[x^2-(-2)\right]\; dx=9}$};
...
\choice{$\int_0^3 (-1-x^2)\; dx$}\\
\choice{$\int_0^3 (x^2 - 1) \; dx$}\\
\choice[correct]{$\int_0^3 (x^2 + 1) \; dx$}\\
\end{multipleChoice}
...
\addplot[thick, blue, dashed] coordinates {(0,0) (0,1)};
...
%\node at (axis cs: 2.1,-.4){$2$};
\node at (axis cs: .9,-2.8){$\displaystyle{A=\int_{0}^{\pi/2} \left[e^x-\sin(x)\right]\; dx= e^{\pi/2} - 2}$};
...
\node at (axis cs: 2.1,-.4){$2$};
\node at (axis cs: 1.4,-5.2){$\displaystyle{A=\int_{1}^{2} \left[(5-x^2)-(x-4)\right]\; dx=\frac{31}{6}}$};
...
\begin{multipleChoice}
\choice{$\int_0^2 (-x^2 +x -2)\; dx$}\\
\choice{$\int_0^2 (x^2 +x - 2) \; dx$}\\
\choice[correct]{$\int_0^2 (x^2 -x + 2) \; dx$}\\
\end{multipleChoice}
...
Find the area of the region bounded between the curves $y = (3x+2)^3$ and $y = (3x+2)^2$ from $x = 0$ to $x = 1$.\\
The top curve is: $y = \answer{(3x+2)^3}$.\\
The bottom curve is: $y = \answer{(3x+2)^2}$.\\
The definite integral that represents this area is: 

\begin{multipleChoice}
\choice{$\int_0^1 \left[(3x+2)^2  -(3x+2)^3\right]\; dx$}\\
\choice[correct]{$\int_0^1 \left[(3x+2)^3  -(3x+2)^2\right] \; dx$}\\
\choice{$\int_0^1 (3x+2) \; dx$}\\
\end{multipleChoice}
...
\node at (axis cs: .785,-.2){$\pi/4$};
\node at (axis cs: .5,-.6){$\displaystyle{A=\int_{1}^{\pi/4} \left[\cos(x)-\sin(x)\right]\; dx=\sqrt2-1}$};
...
\node at (axis cs: .5,-1.5){$\displaystyle{A=\int_{0}^{1} \left(\sqrt x- x^2\right)\; dx=\frac{1}{3}}$};
...
\node at (axis cs: 0,-6.6){$\displaystyle{A=\int_{-1}^{2} \left[(2x - 1) - (x^2 + x - 3)\right]\; dx=\frac{9}{2}}$};
...
\begin{problem}(problem 6b)  Find the area of the region bounded by the parabolas $y = x^2 - 4$ and $y = 8 - 2x^2$.\\
The points of intersection (from smallest to largest) are: $x = \answer{-2}$ \; and \; $x = \answer{2}$.\\
...
The $x$-coordinates of the points of intersection are (in increasing order):
\[
x= \answer{-1}, \quad x = \answer{0} \quad \mbox{and} \quad x = \answer{1}
\]
...
Find the area of the region enclosed by the parabolas $x = y^2 - 1$ and $x = 7 - y^2$.\\
