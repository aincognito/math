...
\begin{center}
\begin{tikzpicture}
\begin{axis}[axis x line=  none, axis y line = none, xtick={pi/2}, 
xticklabels={$\pi$}, title={\textup{Area between} $y=e^x$ and $y=\sin(x)$ 
from $x = 0$ to $x=\pi/2$}]
...
\node at (axis cs: 1.4,-2.8){$\displaystyle{A=\int_{a}^{b} \left[f(x)-g(x)\right]\; dx= e^{\pi/2} - 2}$};
...
The area of the region is $\answer{617/12}$.
\end{problem}
