...
Cylinders, cones and spheres are familiar examples of solids of revolution. 
...
\[
V_x = \pi R(x)^2 h  = \pi \left(\frac{r}{h} x\right)^2 \; dx = \pi \frac{r^2}{h^2} x^2 \; dx.
\]
...
\[
V = \int_0^h V_x \; dx = \int_0^h \pi R(x)^2 \; dx = \int_0^h \pi \frac{r^2}{h^2} x^2 \; dx 
\]
\[
...
\[
v_x = \pi R(x)^2 dx = \pi y^2 \; dx = \pi \left(\sqrt{r^2 - x^2}\right)^2 dx = \pi (r^2 - x^2)\; dx.
\]
...
\[
V = \int_{-r}^r V_x \; dx = \int_{-r}^r \pi R(x)^2 \; dx = \int_{-r}^r \pi (r^2 - x^2) \; dx 
\]
\[
...
\begin{example}[example 4] Find the volume of the solid obtained by revolving the region bounded by the graphs of $y=\sqrt, y=0, x=0$, and $x = 1$
about the $x$-axis.\\
We begin with a sketch of the region.
...
\[
A_x = \pi (R^2 - r^2) h = \pi \left[3^2 - \left(e^x\right)^2\right] \; dx = \pi \left(9 - e^{2x}\right) \; dx.
\]
The volume of the solid can be otained by integrating $A_x \, dx$ as $x$ ranges from $0$ to $\ln 3$:
\[
V = \int_0^{\ln 3} V_x \; dx = \int_0^{\ln 3} \pi \left(9 - e^{2x}\right) \; dx = \pi \left(9x - \frac{e^{2x}}{2}\right)\bigg|_0^{\ln 3} = \left(9\ln(3)-4\right)\pi.
\]
...
\begin{example}[example 7] Find the volume of the solid obtained by revolving the region bounded by the graphs of $y = x^2, y=0$, and $x = 1$ about the $y$-axis.\\
We begin with a sketch of the region.
...
radius, $r = x$, height, $h = 2 - \sqrt x$, and thickness, $dx$. 
