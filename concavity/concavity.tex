...
\begin{image}
\begin{tikzpicture}
\begin{axis}[axis lines = center,  axis y line=none, xmin=-2.5, xmax = 1, xtick={-1.138, -0.195}, xticklabels={$\alpha$, $\beta$}, ytick={}]
\addplot[smooth,black, domain=-2.5:1, thick, <->] {0};
%\addplot[blue, mark = *] coordinates{(-4,166)} node[right,blue] {inflection point};
\node at (axis cs:-0.75, 0.45){Sign of $f''(x)$ };
\node at (axis cs:-1.8, 0.15){ + \ + \ +  };
\node at (axis cs:-0.65, 0.15){ - \ - \ - \ - };
\node at (axis cs:0.5, 0.15){ + \ + \ +  };
%\node[blue] at (axis cs:0.2, 50){\begin{tabular}{c} concave\\up \end{tabular}};
