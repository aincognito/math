...
Find the derivastive of the power series function $\dis f(z) = \sum_{k=0}^\infty \frac{z^k}{k!}$\\
...
\]
and reindexing so that $k$ starts at $0$ instead of $1$ we get
\[
...
$\dis f'(z) = \sum_{k=0}^\infty \answer{\frac{(-1)^k}{(2k)!}z^k}$
\end{problem}
