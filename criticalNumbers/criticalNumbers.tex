...
In this section we will find the \textbf{critical numbers} of a given function. 
Critical numbers come in two types and the idea behind the definition comes from the 
...
\addplot[red,domain=0.87:1,thick] {2-(x+1)^2};
\addplot[red,domain=1:1.2,thick] {2.5*x-4.5};
\addplot[blue, mark = *] coordinates{(-1,2)} ;
\addplot[red, thick, domain=-5/3:-1/3]{2} node[red,above, pos = 0] {\begin{tabular}{c}  horizontal \\ tangent line \end{tabular}};
\addplot[blue, mark = *] coordinates{(1,-2)} node[below, right, red]{corner point};
\end{axis}
\end{tikzpicture}
\end{image}
...
\begin{remark}
Type 1 critical numbers correspond to horizontal tangent lines in the graph of the function. 
Local extremes are frequently found at these points.  
\end{remark}
\begin{remark}
Type 2 critical numbers typically correspond to corner points or vertical tangent lines. 
Local extremes can also occur at these points.
\end{remark}
...
\addplot[red, thick, domain=-1.2:1.2]{2} node[pos=1.4]{\begin{tabular}{c} horizontal \\ tangent line \end{tabular}};

\node[color=blue] at (axis cs:-1.6,-1.6) {$y = 2-x^2$};
\end{axis}
\end{tikzpicture}
\hspace{1 in}
\begin{tikzpicture}
...
\addplot[red, thick, domain=-1.2:1.2]{1} node[above,pos=-0.1]{\begin{tabular}{c} horizontal \\ tangent line \end{tabular}};
...
\addplot[blue,mark = *] coordinates {(0,-1)};
\addplot[red, thick] coordinates {(0,-2.2)(0,0.2)} node[right, pos = 0.3]{vertical tangent line};
...
\addplot[domain=-0.5:0, samples = 100, color=red, thick]{-3/2*x-2};
\addplot[domain=0:0.5, samples = 100, color=red, thick]{3/2*x-2} ;

\addplot[blue,mark = *] coordinates {(0,-2)}node[right, red]{corner point};
\node[color=blue] at (axis cs:-1.5,2) {$y = |x|-2$};
\end{axis}
\end{tikzpicture}
...
\begin{image}
\begin{tikzpicture}
\begin{axis}[axis x line =center, axis y line = center, xmin = -5.5, xmax = 6.5, ymin = -24.9, ymax = 20, xtick = {-2, 3}, xticklabels={-2,3}, 
ytick={-10, 10},yticklabels={}, title = {Type 1 critical numbers at $x = -2$ and $x = 3$}]
%ymin=-14, ymax = 11
\addplot[domain=-5:6, samples = 100, color=blue, thick]{2*x^3 - 3*x^2 -36*x + 6)/6} ;
...
\addplot[thick, white] coordinates{(0,-15) (0,-25)};
\node[color=blue] at (axis cs:-1.5,-19) {$y = \frac{x^3}{3} - \frac{x^2}{2} - 6x +1$};
\end{axis}
\end{tikzpicture}
\end{image}
...
\node[color=blue] at (axis cs:2.5,25) {$y = x^4 - 4x^3 + 2$};
\end{axis}
\end{tikzpicture}
\end{image}
...
\begin{image}
\begin{tikzpicture}
\begin{axis}[axis x line=  center, axis y line = center,xmin = -2.5, xmax = .5, ymin = -0.05, ymax =0.28,
xtick={-2/3},xticklabels={$-\frac23$}, ytick={1/6},yticklabels={$\frac16$},
title={Type 1 critical numbers at $x = -2/3$ and $x = 0$}];

\addplot[domain=-2.4:0.3, samples = 100, color=blue, thick]{x^2*e^(3*x)} ;
...
\node[color=blue] at (axis cs:-1.9,0.03) {$y = x^2e^{3x}$};
\end{axis}
\end{tikzpicture}
\end{image}
...
\node[color=blue] at (axis cs:4,1.2) {$y = \sqrt[3] x$};
...
\begin{image}
\begin{tikzpicture}
\begin{axis}[axis x line=  center, axis y line = center, xmin=-4.5, xmax = 4.5, ymin=-10.5, ymax = 10.5,
xtick={}, xticklabels={},ytick={},yticklabels={}, title={No critical numbers}]

\addplot[domain=-4:-.1, samples = 100, color=blue, thick]{1/x} ;
\addplot[domain=.1:4, samples = 100, color=blue, thick]{1/x} ;
\node[color=blue] at (axis cs:3,2) {$y = \frac{1}{x}$};
\addplot[dashed,red, thick] coordinates {(0,-10.5)(0,10.5)} node[pos=0.1,right,red]{\begin{tabular}{c}  vertical asymptote \\ at $x=0$ \end{tabular}};
\end{axis}
\end{tikzpicture}
\end{image}
